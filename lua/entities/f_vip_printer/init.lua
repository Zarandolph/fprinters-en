...
function ENT:Initialize()
	...
	self:SetColor( fprinters.config.vipcolor )
	...
	self.yaniyor = false
	timer.Simple(fprinters.config.vipprinttime, function() if IsValid(self) then yaratulan(self) end end)
end
...
function ENT:SaglikArttir( miktar )
	if (self:GetSaglik()+miktar)>250 then 
		self:SetSaglik(250)
	else
	self:SetSaglik( self:GetSaglik()+miktar )
    end
...
function ENT:ParaYarat()
	...
	local printamount = fprinters.config.vipprintamount
	local printtime = fprinters.config.vipprinttime
...
function ENT:Touch( ent )
	...
	elseif (ent:GetClass()=="fprinters_healthpack") then
		...
		if self.yaniyor then return end
		if self:GetSaglik()==250 then return end
		ent:Remove()
